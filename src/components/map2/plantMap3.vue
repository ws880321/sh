...
export default {
  ...
  methods: {
    initMap() {
      ...
      // const cloudmask = new L.LeafletGeotiff("tif/af.tif", {
      //   band: 0,
      //   opacity: 1,
    ...
    creatHeat(data) {
      ...
      // this.layers= new L.LeafletGeotiff('tif2/'+data+'.tif', {
      // this.layers= new L.LeafletGeotiff('tif3/DWB.tif', {
        opacity: 1,
        renderer: new L.LeafletGeotiff.Plotty({
            band:1,
            colorScale: 'ccc',
            ...
            clampLow: false,
            clampHigh: true,
            displayMin: this.max[1],
            displayMax: this.max[0]
        })
      }).addTo(this.map);
    }
  },
