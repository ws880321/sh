  ...
  >
   <span slot="text" slot-scope="text">
    ...
    <span slot="action" slot-scope="text,record"  class="op-box"> 
      <!-- <slot :record="record"> -->
         <a href="javascript:;" @click="() => details(record,2)" >执行</a> 
         <a href="javascript:;" @click="() => details(record,1)" >评价</a>  
    ...
    <span slot="c" slot-scope="text,record"  class="c"> 
      {{c}}
    </span>
...
export default {
  ...
  data() {
    return {
      columns: [],
      cdata: [],
      c:sessionStorage.getItem('userInfo')
    };
  },
  props: {
    title: {
